...
vim.o.list = false -- JE: no listchars to start with
...
vim.o.scrolloff = 10 -- aka :se so=10
...
vim.api.nvim_create_autocmd("FileType", {
	desc = "Spelling and linebreaking in plaintext-like files",
	group = vim.api.nvim_create_augroup("plaintext_types", { clear = true }),
...
require("lazy").setup({
	...
	},
	{
		...
		config = function()
			...
			vim.diagnostic.config({
				...
				cunderline = { severity = vim.diagnostic.severity.ERROR },
	...
	{ -- Autocompletion
		...
		opts = {
			keymap = {
				...
				preset = "default",
				["<C-k>"] = false, -- or {}, let digraph use be normal and don't break it plz
				["<C-K>"] = { "show_signature", "hide_signature", "fallback" }, -- put blink behind capital ctrl-k
	...
	{ -- Parser manager (highlight/indent now built into NeoVim 0.12+)
		"nvim-treesitter/nvim-treesitter",
		branch = "main", -- master is archived; main has NeoVim 0.12 compat
		build = ":TSUpdate",
		config = function()
			...
			vim.api.nvim_create_autocmd("VimEnter", {
				once = true,
				callback = function()
					local ensure = { "bash", "c", "diff", "go", "html", "lua", "luadoc",
